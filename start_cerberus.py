...
def main(cfg):
    ...
    if os.path.isfile(cfg):
        ...
        prometheus_alerts = config["cerberus"].get("prometheus_alerts", [])
        ...
        while int(iteration) < iterations:
            ...
            try:
                ...
                # Check configured prometheus alerts
                if promcli.prom_cli:
                    for alert in prometheus_alerts:
                        try:
                            query = alert["expr"]
                            description = alert.get("description", query)
                            severity = alert.get("severity", "warning")
                        except (TypeError, KeyError) as e:
                            logging.warning("Skipping malformed prometheus alert entry: %s\n" % e)
                            continue
                        metrics = promcli.process_prom_query(query)
                        if metrics:
                            logging.warning("[%s] %s\n" % (severity, description))
                            logging.info("%s\n" % (metrics))
                            names = [m["metric"].get("pod", m["metric"].get("instance", query)) for m in metrics]
                            dbcli.insert(datetime.now(), time.time(), 1, description, names, "prometheus alert")
